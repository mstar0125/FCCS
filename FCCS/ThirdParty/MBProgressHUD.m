...
+ (void)showGlobalHUDWithTitle:(NSString *)title {
    if ([[NSThread currentThread] isEqual:[NSThread mainThread]]) {
        ...
        globalHUD = [MBProgressHUD showHUDAddedTo:window animated:YES];
        globalHUD.labelText = title;
    } else {
...
+ (void)showGlobalHUDWithTitleNotAnimated:(NSString *)title {
    if ([[NSThread currentThread] isEqual:[NSThread mainThread]]) {
        if (globalHUD.alpha != 0) {
            globalHUD.labelText = title;
        } else {
            UIWindow *window = [[UIApplication sharedApplication].delegate window];//[[[UIApplication sharedApplication] windows] lastObject];
            [MBProgressHUD hideGlobalHUDNotAnimated];
            globalHUD = [MBProgressHUD showHUDAddedTo:window animated:NO];
            globalHUD.labelText = title;
        }
    } else {
        dispatch_sync(dispatch_get_main_queue(), ^{
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {	
	...
	if (UIInterfaceOrientationIsLandscape(orientation)) {
        if (systemVersion >= 8.0) {
            if (orientation == UIInterfaceOrientationLandscapeLeft || orientation == UIInterfaceOrientationLandscapeRight) { radians = 0; }
